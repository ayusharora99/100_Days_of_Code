...
summary(statcast2019)

data = statcast2019[ ,c("type" ,"events","launch_speed", "launch_angle")]

# only X
unique(data$type)

data = subset(data, type == 'X')
...
data_outs['events'] = "Out"
unique(data_outs$events)

data = rbind(data_outs, data_base_hits, data_homeruns)

unique(data$events)

```
Training & Testing for events
```{r}
```
